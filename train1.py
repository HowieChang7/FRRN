...
def train_generator():  # 加载图片
    ...
    while True:
        ...
        for id in ids_train_batch.values:
            batch += 1
            img = cv2.imread('input1/train/{}.jpg'.format(id))
...
def train():
    ...
    plt.figure()
    N = EPOCHS
    plt.plot(np.arange(0, N), H.history["dice_coeff_multilabel"], label="train_dice_coeff")
    plt.plot(np.arange(0, N), H.history["val_dice_coeff_multilabel"], label="val_dice_coeff")
    plt.title("Training dice_coeff and Valid dice_coeff")
    plt.xlabel("Epoch #")
