...
df_test = pd.read_csv('input2/sample_submission.csv')
...
def predict():
    ...
    for start in tqdm(range(0, len(ids_test), batch_size)):
        ...
        for id in ids_test_batch.values:
            batch += 1
            img = cv2.imread('input2/test/{}.jpg'.format(id))
            # plt.imshow(img)
            # plt.title("Test Image Input")
            # plt.show()
            img = np.array(img, dtype="float") / 255.0
            x_batch.append(img)
            label = cv2.imread("input2/test_masks/{}_mask.png".format(id), cv2.IMREAD_GRAYSCALE)
            # plt.imshow(label)
            # plt.title("Test Image label Input")
            # plt.show()
